...
async function migratePricingBundles() {
  try {
    ...
    console.log(
      `✅ Converted ${multiplePaymentsResult.rowCount || 0} users to complete bundle`,
    );
    ...
    const indexes = [
      ...
      {
        name: "idx_user_payment_certificate_search",
        columns: "purchased_certificates",
      },
    ];
    ...
    for (const index of indexes) {
      try {
        ...
        console.log(`⚠️  Index ${index.name} might already exist:`, error);
      }
    }
    ...
    console.log("\n📊 Summary:");
    console.log(
      "- Added bundle_type, certificate_count, purchased_certificates columns",
    );
